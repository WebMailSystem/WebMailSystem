...
public interface InboxRepository extends JpaRepository<Inbox, InboxId>{
    ...
    @Query("SELECT i FROM inbox i WHERE i.id.repositoryName = :repositoryName AND i.sender LIKE CONCAT(:sender,'%') AND i.messageBody LIKE CONCAT('%','Subject:','%',:subject,'%')" )
    Inbox findByRepositoryNameAndSenderAndMessageBody(@Param("repositoryName")String repositoryName,@Param("sender")String sender,@Param("subject")String subject);
}
